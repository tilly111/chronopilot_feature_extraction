...
def process_single_ppg_file(file_path):
    """
    Processes a single PPG file and extracts features.
    ...
    # Ensure the 'PPG' column exists
    if "PPG" not in data.columns:
        raise KeyError(f"Die Datei '{file_name}' enthält keine 'PPG'-Spalte.")

    # Extract the PPG signal
    raw_signal = data["PPG"]

    # Process the PPG signal
    processed_signal, info = nk.ppg_process(raw_signal, sampling_rate=SAMPLING_RATE)

    # Extract features
    features_df = nk.ppg_analyze(processed_signal, sampling_rate=SAMPLING_RATE, method="interval-related")
...
def process_ppg_files_in_folder(filtered_folder, output_folder):
    """
    Processes all PPG files in the specified folder and saves the extracted features.
    
    Parameters:
        filtered_folder (str): Path to the folder containing PPG files.
    ...
    for root, _, files in os.walk(filtered_folder):
        for file in files:
            if file.endswith("_PPG.csv"):  # Process only files ending with '_PPG.csv'
                file_path = os.path.join(root, file)
                try:
                    features = process_single_ppg_file(file_path)
                    ppg_features.append(features)
                except KeyError as e:
                    print(f"Warnung: {e}")  # Warn about missing 'PPG' column
                except Exception as e:
                    print(f"Fehler bei der Verarbeitung der Datei {file}: {e}")

    # Save the results
    output_file = os.path.join(output_folder, "ppg_features.csv")
    df = pd.DataFrame(ppg_features)
